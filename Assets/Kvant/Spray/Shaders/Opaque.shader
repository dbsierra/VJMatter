...
Shader "Kvant/Spray/Opaque PBR"
{
    ...
    SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
           ...
           // float4 pos = tex2D(_PositionBuffer, IN.uv_MainTex);

           // float xpos = IN.worldPos.x + IN.worldPos.z;

           // float r = cos(_Time.y + _Time.y*.1*6.28 + xpos )*.5+.5;
            //float g = cos(_Time.y + .33*6.2 + xpos )*.5+.5;
           // float b = cos(_Time.y + .66*6.28 + xpos )*.5+.5;
